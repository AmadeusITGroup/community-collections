#!/usr/bin/env python3
"""
GitHub Issues Triage Script

Exports open issues, labels, and templates from a GitHub repository,
...
PRIORITY_KEYWORDS = {
    PRIORITY_CRITICAL: [
        ...
        r"\bCVE-\d+", r"\bdata\s*loss\b", r"\bproduction\s*(down|broken|crash)",
        r"\bblocking\b", r"\boutage\b",
    ],
    PRIORITY_HIGH: [
        r"\bhigh\s*priority\b", r"\bimportant\b", r"\bregression\b",
        r"\bbreaking\b", r"\bcrash(es|ing)?\b", r"\bfails?\b", r"\bbug\b",
        r"\berror\b", r"\bbroken\b",
    ],
    PRIORITY_MEDIUM: [
        r"\bmedium\s*priority\b", r"\benhancement\b", r"\bimprovement\b",
        r"\bfeature\s*request\b", r"\bwould\s*be\s*nice\b",
    ],
    PRIORITY_LOW: [
        r"\blow\s*priority\b", r"\bminor\b", r"\btypo\b", r"\bdocumentation\b",
        r"\bcosmetic\b", r"\bnit\b", r"\bnice\s*to\s*have\b",
    ],
}
...
def _is_already_triaged(issue: Dict[str, Any], triage_label_patterns: Sequence[str]) -> bool:
    """Check if an issue appears to have been already triaged."""
    labels = [str(l.get("name", "")).lower() for l in (issue.get("labels") or []) if isinstance(l, dict)]
    
    if not labels:
        return False
    
    # If issue has labels that are NOT just "needs-triage" type labels, consider it triaged
...
def _download_issue_templates(repo: str, out_dir: Path) -> List[Path]:
    ...
    listing = None
    try:
        listing = _gh_api_json(f"repos/{repo}/contents/.github/ISSUE_TEMPLATE")
    except Exception:
        listing = None
    ...
    if isinstance(listing, list):
        for item in listing:
            ...
            if not name:
                continue
            raw = _try_gh_api_raw(f"repos/{repo}/contents/.github/ISSUE_TEMPLATE/{name}")
            if raw is None:
                continue
...
def _make_proposals(
    ...
    for issue in issues:
        ...
        existing_labels = [str(l.get("name")) for l in (issue.get("labels") or []) if isinstance(l, dict) and l.get("name")]

        # Check if already triaged
        already_triaged = _is_already_triaged(issue, triage_label_patterns)
        
        # Find potential duplicates
        potential_duplicates = _find_duplicates(issue, issues)
        ...
        # Skip detailed triage for already-triaged issues
        if already_triaged:
            reasons.append("Issue appears already triaged (has substantive labels)")
            proposals.append(
                Proposal(
                    number=number,
                    url=url,
        ...
        if not existing_labels and needs_triage_label:
            ...
            reasons.append("Issue has no labels")
        ...
        if missing_sections:
            reasons.append("Issue appears not to follow the issue template")
            ...
            proposed_comment = (
                "Thanks for opening this issue. To help us triage it faster, could you please update the issue to follow the "
                "repository's issue template and include the missing sections"
...
def _write_apply_script(repo: str, proposals: Sequence[Proposal], out_dir: Path) -> Path:
    ...
    for p in proposals:
        if p.proposed_labels:
            cmd = ["gh", "issue", "edit", "-R", "${REPO}", str(p.number)]
        ...
        if p.proposed_comment.strip():
            ...
            lines.append(f"gh issue comment -R ${{REPO}} {p.number} --body {comment}")
...
def _write_report(repo: str, proposals: Sequence[Proposal], out_dir: Path) -> Path:
    ...
    lines: List[str] = []
    lines.append(f"# Issues Triage Report for `{repo}`")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"- **Total open issues:** {len(proposals)}")
    ...
    lines.append("## Issues Needing Triage")
    lines.append("")
    if needs_triage:
        lines.append("| Issue | Title | Priority | Proposed Labels | Duplicates | Reasons |")
        lines.append("|---:|---|:---:|---|---|---|")
        for p in sorted(needs_triage, key=lambda x: [PRIORITY_CRITICAL, PRIORITY_HIGH, PRIORITY_MEDIUM, PRIORITY_LOW].index(x.proposed_priority)):
            ...
            issue_link = f"[{p.number}]({p.url})" if p.url else str(p.number)
            ...
            lines.append(f"| {issue_link} | {title} | {priority_emoji} {p.proposed_priority} | {labels} | {dups} | {reasons} |")
    else:
        lines.append("*No issues need triage.*")
    lines.append("")
    ...
    if with_duplicates:
        ...
        lines.append("")
        lines.append("| Issue | Title | Similar To | Similarity |")
        lines.append("|---:|---|---|---:|")
        for p in with_duplicates:
            ...
            for dup in p.potential_duplicates[:2]:
                ...
                lines.append(f"| {issue_link} | {title} | {dup_link} | {sim_pct} |")
        lines.append("")
    ...
    if already_triaged:
        ...
        lines.append("")
        lines.append(f"*{len(already_triaged)} issues were skipped because they already have substantive labels.*")
        lines.append("")
        lines.append("<details>")
        lines.append("<summary>View skipped issues</summary>")
        lines.append("")
    ...
    lines.append("- Already-triaged issues are excluded from `apply.sh`.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export GitHub issues/labels/templates and generate a triage report + apply script.")
    ...
    issues = _list_open_issues(args.repo)
    template_paths = _download_issue_templates(args.repo, out_dir)

    _write_json(out_dir / "labels.json", labels)
    _write_json(out_dir / "issues.json", issues)

    available_label_names = [str(l.get("name")) for l in labels if isinstance(l, dict) and l.get("name")]
    proposals = _make_proposals(args.repo, issues, available_label_names, template_paths)
    proposals_json = [
        {
            "number": p.number,
            "url": p.url,
    ...
    # Only include non-triaged issues in apply script
    ...
    print(f"  Total open issues: {len(proposals)}")
